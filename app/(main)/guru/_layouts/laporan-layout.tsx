...
import LaporanUpdateDialog from "../../_components/laporan-update-dialog";
...
export default function LaporanLayout({ angkatanId }: IProps) {
  ...
  const [actionType, setActionType] = useState<'approve' | 'decline' | 'edit-note'>('approve');
  ...
  const fetch = useCallback((
    ...
    axios
      .get(`/api/angkatan/laporan/${angkatanId}`, {
        params: {
          q: keyword,
    ...
    <div className="space-y-4">
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem
                                disabled={laporan.status === 'TERKIRIM'}
                                onClick={() => {
                                  setActionType('edit-note');
                                  setOpenActionDialog(true);
                                  setSelectedLaporan(laporan);
                                }}
                              >
      ...
      <LaporanUpdateDialog
        ...
        onOpenChange={(open) => {
          ...
          setOpenActionDialog(open);
        }}
        type={actionType}
        selectedLaporan={selectedLaporan}
        onSuccess={((laporan) => {
          setData((prev) => prev.map((item) => {
            if (item.id === laporan.id) return laporan;
            return item;
          }))
        })}
      />
